...
% Original Vector Field Topology (2D)
...
fprintf("Displaying original vector field topology of test field %d\n",ch);
...
for m = 1:N_init_guess
    counter = counter + 1;
    xc(counter) = X(ind_s(m));
    yc(counter) = Y(ind_s(m));
...
pbaspect([1 1 1]);
saveas(gcf, 'Original_Topology.png');
